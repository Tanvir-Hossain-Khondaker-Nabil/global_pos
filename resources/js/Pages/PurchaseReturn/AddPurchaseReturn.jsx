...
export default function Create({ 
    ...
    // Load initial items when purchase changes
    useEffect(() => {
        if (initialItems && initialItems.length > 0) {
            // Map the initial items to include all necessary properties
            ...
            // Update form data with purchase ID
            form.setData('purchase_id', purchase?.id || '');
        } else {
            setReturnItems([]);
        }
    }, [initialItems, purchase]);
    ...
    const handleReturnItemChange = (index, field, value) => {
        ...
        if (field === 'return_quantity') {
            ...
            const maxQuantity = updated[index].max_quantity || 0;
    ...
    const validateForm = () => {
        ...
        if (!form.data.purchase_id) {
            newErrors.purchase_id = 'Please select a purchase';
        }
        ...
        if (!form.data.return_date) {
            newErrors.return_date = 'Please select return date';
        }
        ...
        returnItems.forEach((item, index) => {
            if (item.return_quantity > 0 && item.return_quantity > item.max_quantity) {
                newErrors[`item_${index}`] = `Quantity exceeds available stock (Max: ${item.max_quantity})`;
            }
        });
    ...
    const handleSubmit = (e) => {
        ...
        form.post(route('purchase-returns.store'), {
            ...
            preserveScroll: true,
            onSuccess: () => {
                router.visit(route('purchase-returns.list'));
            },
            onError: (errors) => {
            ...
            <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                >
                    <div className="flex items-center gap-3">
                        <button
                            onClick={() => router.visit(route('purchase-returns.list'))}
                            ...
                            <div className="card bg-base-100 border border-base-300 mb-6">
                                <div className="card-body">
                                    ...
                                    <div className="form-control">
                                        ...
                                        <select
                                            className={`select select-bordered w-full ${errors.purchase_id ? 'select-error' : ''}`}
                                        ...
                                        >
                                            ...
                                            {recentPurchases.map(p => (
                                                <option key={p.id} value={p.id}>
                                                    {p.purchase_no} - {p.supplier?.name} - {p.purchase_date} - ৳{formatCurrency(p.grand_total)}
                                                </option>
                                            ))}
                                        </select>
                                        {errors.purchase_id && (
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="font-mono">{selectedPurchase.purchase_no}</div>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div>
                                                        ...
                                                        {selectedPurchase.supplier?.company && (
                                                            <div className="text-xs text-gray-500">{selectedPurchase.supplier.company}</div>
                                                        )}
                                                    </div>
                                                </div>
                                        ...
                                        <div className="lg:col-span-1 space-y-4">
                                            <div className="form-control">
                                                ...
                                                <div className="grid grid-cols-2 gap-2">
                                                    <label className={`card card-compact cursor-pointer ${returnType === 'money_back' ? 'bg-primary text-primary-content' : 'bg-base-100'}`}>
                                                        <div className="card-body p-3">
                                                            <div className="flex items-start">
                                                                ...
                                                                <div className="ml-3">
                                                                    ...
                                                                    <p className="text-xs mt-1">Refund amount to supplier</p>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </label>
                                                    
                                                    <label className={`card card-compact cursor-pointer ${returnType === 'product_replacement' ? 'bg-warning text-warning-content' : 'bg-base-100'}`}>
                                                        <div className="card-body p-3">
                                                            <div className="flex items-start">
                                                                <input
                                                                    type="radio"
                                                                    ...
                                                                    onChange={() => {
                                                                        ...
                                                                        form.setData('return_type', 'product_replacement');
                                                                    }}
                                                                />
                                                                <div className="ml-3">
                                                                    <div className="font-medium">Product Replacement</div>
                                                                    <p className="text-xs mt-1">Replace with other products</p>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </label>
                                            ...
                                            <div className="form-control">
                                                ...
                                                <input
                                                    type="date"
                                                    className={`input input-bordered w-full ${errors.return_date ? 'input-error' : ''}`}
                                                    value={form.data.return_date}
                                                    onChange={(e) => form.setData('return_date', e.target.value)}
                                                    required
                                                />
                                                {errors.return_date && (
                                                    <div className="text-error text-sm mt-1">{errors.return_date}</div>
                                                )}
                                            </div>
                                            ...
                                            {returnType === 'money_back' && (
                                                <div className="form-control">
                                                    ...
                                                    {errors.account_id && (
                                                        <div className="text-error text-sm mt-1">{errors.account_id}</div>
                                                    )}
                                                </div>
                                            )}
                                                ...
                                                <div className="card-body">
                                                    ...
                                                    {returnItems.length > 0 ? (
                                                                        ...
                                                                        <div className="flex justify-between items-start mb-3">
                                                                            <div>
                                                                                ...
                                                                                <div className="flex gap-4 mt-2 text-xs text-gray-500">
                                                                                    <span>Purchased: {item.purchase_quantity}</span>
                                                                                    <span>Stock: {item.stock_quantity}</span>
                                                                                    <span>Already Returned: {item.already_returned}</span>
                                                                        ...
                                                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                                                            <div className="form-control">
                                                                                ...
                                                                                <input
                                                                                    ...
                                                                                    onChange={(e) => {
                                                                                        console.log('Quantity changed for item', index, 'value:', e.target.value);
                                                                                        handleReturnItemChange(index, 'return_quantity', e.target.value);
                                                                                    }}
                                                                                />
                                                                                {errors[`item_${index}`] && (
                                                                                    <div className="text-error text-xs mt-1">{errors[`item_${index}`]}</div>
                                                                                )}
                                                                            </div>
                                                                            ...
                                                                            <div className="form-control">
                                                                                ...
                                                                                <div className="font-medium">
                                                                                    ৳{formatCurrency((item.return_quantity || 0) * (item.unit_price || 0))}
                                                                                </div>
                                                                            </div>
                                                        ...
                                                        <div className="text-center py-8">
                                                            <PackageOpen size={32} className="mx-auto text-gray-300 mb-2" />
                                                    ...
                                                    <div className="mt-4 pt-4 border-t border-base-300">
                                                        <div className="flex justify-between items-center">
                                                            ...
                                                            <span className="font-semibold text-lg">৳{formatCurrency(totals.totalReturn)}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div className="card-body">
                                                    ...
                                                    <div className="space-y-3 mt-4">
                                                        <div className="flex justify-between">
                                                            ...
                                                            <span className="font-medium">৳{formatCurrency(totals.totalReturn)}</span>
                                                        </div>
                            ...
                            <div className="flex justify-between items-center">
                                <div>
                                    <p className="text-sm text-gray-500">
                                        {returnItems.filter(item => item.return_quantity > 0).length} items selected • Total: ৳{formatCurrency(totals.totalReturn)}
                                    </p>
                                </div>
