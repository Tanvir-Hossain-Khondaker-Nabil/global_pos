...
export default function AddPurchase({
  ...
  }, [adjustFromAdvance, availableAdvance, calculateTotal, manualPaymentOverride, paidAmount]);
      ...
      if (selectedBrand) {
        filtered = filtered.filter((product) => {
          ...
          return product.variants?.some(
            (variant) => variant.attribute_values && Object.keys(variant.attribute_values).includes(selectedBrand)
          );
        });
      }
  ...
  const addItem = (product, variant) => {
    ...
    if (hasAttributes) {
      ...
      const sortedAttributes = Object.entries(variant.attribute_values || {}).sort(([keyA], [keyB]) =>
    ...
    if (existingItemIndex !== -1) {
      ...
      setUnitQuantities(prev => ({ ...prev, [itemKey]: currentQty + 1 }));
    } else {
  ...
  const updateItem = (index, field, value) => {
    ...
    if (field === "unit_quantity" || field === "quantity") {
      ...
      const numericValue = parseFloat(value) || 0;
      updated[index][field] = numericValue;
      updated[index].total_price = (updated[index].unit_quantity || 1) * numericValue;
  ...
  const submit = (e) => {
      ...
      if (form.data.account_id && paidAmount > 0) {
        ...
        if (selectedAccount) {
          if (selectedAccount.current_balance < paidAmount) {
            alert(
              `Insufficient balance in ${selectedAccount.name}. Available: ৳${formatCurrency(
    ...
    form.post(route("purchase.store"), {
      ...
      onError: (errors) => {
        const message = errors.error || errors.advance_adjustment || "Form submission failed";
  ...
  const selectedAccount = form.data.account_id ? accounts.find((acc) => acc.id == form.data.account_id) : null;
  ...
  return (
    <div className={`bg-white rounded-box p-5 ${locale === "bn" ? "bangla-font" : ""}`}>
      <PageHeader
        ...
        subtitle={
          isShadowUser
            ? t("purchase.create_shadow_subtitle", "Add products with shadow pricing")
            : t("purchase.create_subtitle", "Add products with real and shadow pricing")
        }
      >
        <button
                  ...
                  <div className="form-control mb-3">
                    ...
                    {paymentStatus === "unpaid" && (
                      <div className="text-gray-400 text-xs mt-1">
                        Account not required for unpaid purchases
                      </div>
                    )}
                    ...
                    <div key={product.id} className="border-b border-gray-100 last:border-0">
                      ...
                      {product.variants?.map((variant) => {
                        ...
                        const variantMatchesBrand = () => {
                          ...
                          return (
                            attributeKeys.includes(selectedBrand) || product.brand?.name === selectedBrand
                          );
                        };
                        ...
                        <div className="card-body">
                          ...
                          <div className="mt-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <span className="text-xs font-bold text-gray-700">Unit Settings</span>
                            </div>
                            ...
                            {selectedUnit !== (item.sale_unit || selectedUnit) && (
                              <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded">
                                <div className="text-xs text-blue-800">
                                  <strong>Note:</strong> Purchase in {selectedUnit.toUpperCase()}, 
                                  sell in {(item.sale_unit || selectedUnit).toUpperCase()}
                                </div>
                              </div>
                            )}
                          </div>
