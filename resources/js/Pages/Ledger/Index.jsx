...
export default function LedgerIndex({
  ...
  const handleFilter = () => {
    ...
    if (filterForm.data.type && filterForm.data.type !== "all") queryParams.type = filterForm.data.type;

    if (filterForm.data.entity_key) queryParams.entity_key = filterForm.data.entity_key;
    if (filterForm.data.start_date) queryParams.start_date = filterForm.data.start_date;
    if (filterForm.data.end_date) queryParams.end_date = filterForm.data.end_date;

    if (filterForm.data.due_filter && filterForm.data.due_filter !== "all") {
  ...
  const clearFilters = () => {
    ...
    setTimeout(() => {
      router.get(route("ledgers.index"), {}, { preserveScroll: true, preserveState: true });
    }, 0);
  };
          ...
          <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
            <div className="px-6 py-5 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                  <h1 className="text-2xl font-black tracking-tight text-gray-900">Ledger Book</h1>
                  <p className="text-sm text-gray-600 mt-1">
                    Customers & Suppliers summary (ruled ledger view)
                  </p>
                </div>

                <div className="flex flex-wrap items-center gap-2">
                  <StatPill
                    label={type === "supplier" ? "Suppliers" : type === "customer" ? "Customers" : "Entities"}
                    value={type === "customer" ? stats.total_customers : type === "supplier" ? stats.total_suppliers : totalEntities}
                    icon={UserCheck}
                  />
                  <StatPill label="Total Sales" value={`৳${formatCurrency(stats.total_sales_amount)}`} icon={ArrowDownRight} />
                  <StatPill label="Total Purchases" value={`৳${formatCurrency(stats.total_purchases_amount)}`} icon={ArrowUpRight} />
                  <StatPill label="Transactions" value={stats.total_transactions || 0} icon={FileText} />
                </div>
              </div>
            </div>
            ...
            <div className="px-6 py-5">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider">Advanced Filter</h3>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
                ...
                <div className="md:col-span-5">
                  <select
                    ...
                    onChange={(e) => filterForm.setData("entity_key", e.target.value)}
                    className="w-full h-11 px-3 bg-[#fbfaf7] border border-gray-300 rounded-lg
                               focus:ring-2 focus:ring-gray-900/10 focus:border-gray-400 text-gray-800"
                  >
                    <option value="">
                    ...
                    {entityOptions.map((opt, idx) => {
                      if (opt.header) {
                        return (
                          <option key={`h-${idx}`} value="" disabled>
                            ── {opt.header} ──
                          </option>
                        );
                      }

                      if (filterForm.data.type === "customer" && !opt.key.startsWith("customer:")) return null;
                      if (filterForm.data.type === "supplier" && !opt.key.startsWith("supplier:")) return null;
                ...
                <div className="md:col-span-3">
                  <select
                    ...
                    onChange={(e) => filterForm.setData("due_filter", e.target.value)}
                    className="w-full h-11 px-3 bg-[#fbfaf7] border border-gray-300 rounded-lg
                               focus:ring-2 focus:ring-gray-900/10 focus:border-gray-400 text-gray-800"
                  >
                    <option value="all">All (Due + Clear)</option>
                    <option value="due_only">Only Due</option>
                    <option value="clear_only">Only Clear</option>
                  </select>
                </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 bg-[#fbfaf7]">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-sm font-black uppercase tracking-wider text-gray-900">Ledger Entries</h2>
                <p className="text-xs text-gray-600 mt-1">Totals computed from sales/purchases & paid amounts</p>
              </div>
            </div>
          </div>
          ...
          {allEntities.length > 0 ? (
                ...
                <table className="min-w-full">
                  <thead>
                    <tr className="bg-white">
                      {["Entity", "Type", "Contact", "Trans", "Total", "Paid", "Advance", "Due", "Actions"].map((h) => (
                        <th
                          key={h}
                  ...
                  <tbody>
                    {allEntities.map((entity, idx) => {
                      ...
                      const totalAmount =
                        transactions?.reduce((sum, t) => sum + (parseFloat(t?.grand_total) || 0), 0) || 0;

                      const totalPaidAmount =
                        transactions?.reduce((sum, t) => sum + (parseFloat(t?.paid_amount) || 0), 0) || 0;

                      // more accurate due: sum of due_amount (if your filter loads due_amount)
                      const totalDueAmount =
                        transactions?.reduce((sum, t) => sum + (parseFloat(t?.due_amount) || 0), 0) ||
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-5 py-3">
                            <div className="flex items-center gap-3 min-w-0">
                              ...
                              <div className="min-w-0">
                                ...
                                <div className="text-[11px] text-gray-500">ID: {entity.id}</div>
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="px-5 py-3">
                            ...
                            >
                              <span className={`h-2 w-2 rounded-full ${entityConfig.dot}`} />
                          ...
                          <td className="px-5 py-3">
                            <div className="text-sm text-gray-900 font-semibold">
                              {entity.phone ? entity.phone : <span className="text-gray-400">—</span>}
                            </div>
                          </td>

                          {/* Trans */}
                          <td className="px-5 py-3">
                            <div className="text-sm font-black text-gray-900">{transactionCount}</div>
                          </td>

                          {/* Total */}
                          <td className="px-5 py-3">
                            <div className="text-sm font-black text-gray-900">৳{formatCurrency(totalAmount)}</div>
                          </td>

                          {/* Paid */}
                          <td className="px-5 py-3">
                            <div className="text-sm font-black text-gray-900">৳{formatCurrency(totalPaidAmount)}</div>
                          </td>
                          ...
                          <td className="px-5 py-3">
                            <div
                              className={`text-sm font-black ${
                                (advanceAmount || 0) >= 0 ? "text-emerald-700" : "text-rose-700"
                              }`}
                            >
                          ...
                          {/* Due */}
                          <td className="px-5 py-3">
                            <div className="text-sm font-black text-gray-900">৳{formatCurrency(totalDueAmount)}</div>
                          </td>
              ...
              <div className="px-6 py-4 border-t border-gray-200 bg-[#fbfaf7]">
                {type === "all" ? (
                  <div className="space-y-4">
                    {customers &&
                      (Array.isArray(customers) ? customers.length : customers?.data?.length) > 0 && (
                        <div>
                          <h4 className="text-xs font-black uppercase tracking-wider text-gray-700 mb-2">Customers</h4>
                          <Pagination data={customers} />
                        </div>
                      )}

                    {suppliers &&
                      (Array.isArray(suppliers) ? suppliers.length : suppliers?.data?.length) > 0 && (
                        <div>
                          <h4 className="text-xs font-black uppercase tracking-wider text-gray-700 mb-2">Suppliers</h4>
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="text-lg font-black text-gray-900">No entities found</h3>
              <p className="text-sm text-gray-600 mt-2">
                {hasActiveFilters ? "Try adjusting your filters." : "No customers or suppliers available."}
              </p>
              {hasActiveFilters && (
                <button
                  onClick={clearFilters}
            ...
            {type !== "supplier" && salesChartData && (
              <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 bg-[#fbfaf7]">
                  ...
                  <p className="text-xs text-gray-600 mt-1">(Top 10 customers)</p>
                </div>
            ...
            {type !== "customer" && purchasesChartData && (
              <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-200 bg-[#fbfaf7]">
                  ...
                  <p className="text-xs text-gray-600 mt-1">(Top 10 suppliers)</p>
                </div>
                <div className="h-80 p-4">
