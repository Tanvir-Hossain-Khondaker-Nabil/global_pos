...
export default function AddProduct({ category, update, brand, attributes, errors: serverErrors }) {
    ...
    // Search state for attribute keys and values in modal
    ...
    // Filter attributes and their values based on search term
    const getFilteredAttributes = () => {
        ...
        return availableAttributes
            .map(attr => {
                ...
                const attributeNameMatches = attr.name.toLowerCase().includes(searchLower);
                
                // Filter values that match the search term
                const matchingValues = attr.active_values?.filter(val => 
                    val.value.toLowerCase().includes(searchLower)
                ) || [];
                
                // Return attribute if either name matches or has matching values
                if (attributeNameMatches || matchingValues.length > 0) {
                    return {
                        ...attr,
                        // If name matches, show all values; otherwise show only matching values
                        active_values: attributeNameMatches ? attr.active_values : matchingValues
                    };
                }
                return null;
            })
            ...
            <form id="product-form" onSubmit={formSubmit} className="max-w-[1400px] mx-auto px-4 grid grid-cols-12 gap-6">
                ...
                <div className="col-span-12 lg:col-span-4 space-y-6">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col min-h-[400px]">
                        ...
                        {/* Search bar for attributes in the variants card */}
                ...
                {variantAttributeSelector !== null && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 overflow-y-auto">
                        <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col">
                            ...
                            {/* Search bar for attribute keys and values in modal */}
                            <div className="px-6 py-3 border-b bg-white">
                                <div className="relative">
                                    <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400" size={16} />
                                    <input
                                        type="text"
                                        placeholder={t("Search attribute names or values...")}
                                        className="input input-bordered input-sm w-full pl-9"
                            ...
                            <div className="p-6 space-y-6 max-h-[60vh] overflow-y-auto">
                                {getFilteredAttributes().map(attr => (
                                    <div key={attr.code} className="space-y-3">
                                        <h5 className="text-[10px] font-black uppercase text-slate-400 tracking-widest">
                                            {attr.name}
                                            {modalSearchTerm && attr.name.toLowerCase().includes(modalSearchTerm.toLowerCase()) && (
                                                <span className="ml-2 text-[8px] bg-primary/10 text-primary px-1 py-0.5 rounded">
                                                    {t("match")}
                                                </span>
                                            )}
                                        </h5>
                                        <div className="flex flex-wrap gap-2">
                                            {attr.active_values?.map(val => (
                                                <button key={val.id} type="button"
                                                    ...
                                                    {val.value}
                                                    {modalSearchTerm && val.value.toLowerCase().includes(modalSearchTerm.toLowerCase()) && (
                                ...
                                {getFilteredAttributes().length === 0 && (
                                    <div className="text-center py-8 text-slate-400">
                                        {t("No matching attributes or values found")}
                                    </div>
                                )}
                            </div>
