...
export default function Create({ investors = [], outlets = [] }) {
  ...
  const submit = (e) => {
    e.preventDefault();
    post(route("investments.store"), { preserveScroll: true });
  };

  return (
    <div className="p-4 lg:p-8">
      <PageHeader
        title={locale === "bn" ? "নতুন ইনভেস্টমেন্ট" : "Create Investment"}
      ...
      <form onSubmit={submit} className="mt-6 bg-white rounded-2xl border border-slate-100 shadow-sm p-5 max-w-3xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Field
            label={locale === "bn" ? "ইনিশিয়াল টাকার পরিমাণ" : "Initial principal"}
            type="number"
            value={data.initial_principal}
            onChange={(v) => setData("initial_principal", v)}
            error={errors.initial_principal}
            required
          />
