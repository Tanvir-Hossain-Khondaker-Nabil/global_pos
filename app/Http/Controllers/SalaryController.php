...
class SalaryController extends Controller
{
    ...
    {
        $query = Salary::with(['employee' => function($query) {
    ...
    private function calculateEmployeeSalary(Employee $employee, $month, $year)
    {
        ...
        // Notes
        $notes = [];
        if ($totalLateHours > 0) $notes[] = "Late: {$totalLateHours}h";
        if ($totalOvertimeHours > 0) $notes[] = "Overtime: {$totalOvertimeHours}h";
        if ($paidLeaveDays > 0) $notes[] = "Leave: {$paidLeaveDays}d";
        if ($totalBonus > 0) $notes[] = "Bonus: ৳" . number_format($totalBonus, 2);
                ...
                switch ($request->action) {
                    ...
                    case 'pay':
                        if (in_array($salary->status, ['pending', 'approved'])) {
                            ...
                            $count++;
                            Log::info('Salary marked as paid', ['salary_id' => $salary->id]);
                        }
                        break;
